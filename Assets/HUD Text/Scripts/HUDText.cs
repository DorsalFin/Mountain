...
public class HUDText : MonoBehaviour
{
	...
	public void Add (object obj, Color c, float stayDuration)
	{
        ...
        bool isMineral = obj is Tile;
        if (isMineral)
        {
            Tile tile = (Tile)obj;
            val = tile.currentYield;
        }

		// Create a new entry
		Entry ne = Create();
        ne.stay = isMineral ? 999 : 2; // we want it there indefinitely (until we stop hovering)
		ne.label.color = c;
		ne.label.alpha = 0f;
		ne.val = val;
        ne.myObject = obj;
        ...
        else if (isMineral) ne.label.text = ne.val.ToString();
	...
	void Update ()
	{
        ...
        {
            Entry ent = mList[i];
            // object is tile - update mineral count
            if (ent.myObject is Tile)
            {
                Tile tile = (Tile)ent.myObject;
                ent.label.text = tile.currentYield.ToString();
            }
        }
	}
}
